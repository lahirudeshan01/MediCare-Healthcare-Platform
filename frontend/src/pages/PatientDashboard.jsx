...
import { APPOINTMENT_ROUTES, NOTIFICATION_ROUTES, AI_SERVICE } from "../config/api";
...
export function PatientDashboard() {
  ...
  const loadNotifications = async () => {
    ...
    try {
      const response = await fetch(NOTIFICATION_ROUTES.byUser(user.id));
      if (!response.ok) {
  ...
  const markNotificationRead = async (notificationId) => {
    try {
      const response = await fetch(
        NOTIFICATION_ROUTES.markRead(notificationId),
        {
          method: "PATCH",
        },
      );
      ...
      // Ignore notification update failures so the dashboard stays usable.
    }
  };
  ...
  const markAllNotificationsRead = async () => {
    ...
    try {
      const response = await fetch(NOTIFICATION_ROUTES.markAllRead, {
        method: "PATCH",
        headers: {
      ...
      <header className="flex justify-between items-end mb-8">
        ...
        <div className="relative">
          <button
            ...
            className="w-10 h-10 rounded-full bg-white border border-[#D2D2D7]/50 flex items-center justify-center text-[#1D1D1F] hover:bg-[#F5F5F7] transition-colors relative"
          >
            <Bell className="w-5 h-5" />
            {unreadNotificationCount > 0 && (
              <span className="absolute top-2 right-2 min-w-4 h-4 px-1 bg-[#FF3B30] rounded-full border-2 border-white text-[10px] font-semibold text-white flex items-center justify-center">
          ...
          <AnimatePresence>
            {isNotificationPanelOpen && (
              <motion.div
                ...
                className="absolute right-0 mt-3 w-96 max-w-[calc(100vw-2rem)] z-50"
              >
                <GlassCard className="overflow-hidden border border-[#D2D2D7]/50 shadow-xl">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-[#D2D2D7]/50 bg-white/80 backdrop-blur">
                    <div>
                  ...
                  <div className="max-h-[24rem] overflow-y-auto bg-white">
                    {notificationsLoading ? (
                      <div className="px-4 py-6 text-sm text-[#86868B]">
                      ...
                      notifications.map((notification) => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-4 border-b border-[#F0F0F5] transition-colors hover:bg-[#F5F5F7] ${notification.isRead ? "bg-white" : "bg-[#F5F5F7]/60"}`}
                        >
                          <div className="flex items-start gap-3">
                            <div
                              className={`mt-1 w-2.5 h-2.5 rounded-full ${notification.isRead ? "bg-[#D2D2D7]" : "bg-[#0071E3]"}`}
                            />
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-[#4C4C50] mt-1 line-clamp-2">
