...
export function DoctorDashboard() {
  ...
  const doctorApiBase =
    import.meta.env.VITE_DOCTOR_API || "http://localhost:8082";
  ...
  const parseAppointmentDate = (dateValue) => {
    ...
    const parsedWithCurrentYear = new Date(
  ...
  const patientsTodayCount = new Set(
    todaysApprovedAppointments.map((apt) => apt.patientId || apt.patientName),
  ).size;
  ...
  const todaysRevenue = todaysApprovedAppointments.reduce(
    (total, apt) => total + (apt.consultType === "in-person" ? 3000 : 2500),
    0,
  );
  const todaysRevenueLabel = `Rs. ${todaysRevenue.toLocaleString("en-US")}`;
  const selectedPatient =
    patientDirectory.find((patient) => patient.key === selectedPatientKey) ||
    null;
  ...
  const loadPrescriptions = async () => {
    ...
    try {
      const response = await fetch(
        `${doctorApiBase}/api/prescriptions?doctorId=${doctorId}`,
      );
      if (!response.ok) {
  ...
  const loadAppointments = async () => {
    ...
    try {
      const [pendingResponse, approvedResponse] = await Promise.all([
        fetch(
          `${doctorApiBase}/api/appointments?doctorId=${doctorId}&status=pending`,
        ),
        fetch(
          `${doctorApiBase}/api/appointments?doctorId=${doctorId}&status=approved`,
        ),
      ]);
  ...
  const loadAvailability = async () => {
    ...
    try {
      const response = await fetch(
        `${doctorApiBase}/api/doctors/${doctorId}/availability`,
      );
      if (!response.ok) {
      ...
      const mergedSlots = AVAILABILITY_DAYS.flatMap((day) =>
        AVAILABILITY_BLOCKS.map((block) => {
          const existing = incomingSlots.find(
  ...
  const handleEditSchedule = async () => {
    if (!isEditingAvailability) {
      setAvailabilityMessage(
        "Edit mode enabled. Click slots to toggle, then Save Schedule.",
      );
  ...
  const handleAppointmentAction = async (appointmentId, action) => {
    ...
    try {
      const response = await fetch(
        `${doctorApiBase}/api/appointments/${appointmentId}/accept`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleUpdatePrescription = async (id) => {
    ...
    if (
      !editPrescriptionForm.patientName.trim() ||
      !editPrescriptionForm.diagnosis.trim()
    ) {
    ...
    try {
      const response = await fetch(`${doctorApiBase}/api/prescriptions/${id}`, {
        ...
        body: JSON.stringify({
          ...
          medications: multilineTextToMedications(
            editPrescriptionForm.medicationsText,
          ),
        }),
      });
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <section>
                  ...
                  {appointmentMessage && (
                    <p className="text-sm text-[#0071E3] mb-3">
                      {appointmentMessage}
                    </p>
                  )}
                        ...
                        <GlassCard key={req._id} className="p-5">
                          ...
                          <div className="flex gap-2">
                            <AppleButton
                              ...
                              onClick={() =>
                                handleAppointmentAction(req._id, "accept")
                              }
                              icon={<CheckCircle className="w-4 h-4" />}
                            >
                              {isUpdatingAppointmentId === req._id
                                ? "Updating..."
                                : "Accept"}
                            </AppleButton>
                            <AppleButton
                              ...
                              disabled={isUpdatingAppointmentId === req._id}
                              onClick={() =>
                                handleAppointmentAction(req._id, "reject")
                              }
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-4 gap-1 mb-1 pl-12">
                        {AVAILABILITY_BLOCKS.map((block) => (
                          <p
                            key={`header-${block}`}
                            className="text-[11px] font-medium text-[#86868B] text-center"
                          >
                            {block}
                          </p>
                        ))}
                            ...
                            {AVAILABILITY_BLOCKS.map((block) => {
                              ...
                              return (
                                <button
                                  ...
                                  type="button"
                                  onClick={() =>
                                    toggleAvailabilitySlot(day, block)
                                  }
                                  title={block}
          ...
          <div>
            <header className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-[#86868B]">
                  Your available time slots with dates
                </p>
              </div>
              ...
              {upcomingAvailabilityDates.length > 0 && (
                <div className="space-y-4">
                  {upcomingAvailabilityDates.map((item) => (
                    <div
                      key={item.key}
                      className="border border-[#D2D2D7]/60 rounded-xl p-4 bg-white"
                    >
          ...
          <div>
            <header className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-[#86868B]">
                  Appointment requests, medical records, prescriptions, and
                  details
                </p>
              </div>
                        ...
                        <div className="space-y-2">
                          {selectedPatient.appointments
                            ...
                            .map((apt) => (
                              <div
                                key={apt._id}
                                className="rounded-lg border border-[#D2D2D7]/50 p-3 bg-white"
                              >
                        ...
                        <div className="space-y-2">
                          {selectedPatient.appointments
                            ...
                            .map((apt) => (
                              <div
                                key={`record-${apt._id}`}
                                className="rounded-lg border border-[#D2D2D7]/50 p-3 bg-[#F9FAFB]"
                              >
                                <p className="text-sm font-medium text-[#1D1D1F]">
                                  {formatAppointmentDateLabel(
                                    apt.appointmentDate,
                  ...
                  {approvedAppointments.map((apt) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-[#86868B]">
                          {formatAppointmentDateLabel(apt.appointmentDate)},{" "}
                          {apt.appointmentTime}
                        </p>
                        <p className="text-xs text-[#86868B] mt-1">
                          Patient ID: {apt.patientId || "N/A"}
          ...
          <div>
            <header className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-[#86868B]">
                  All prescriptions saved to MongoDB
                </p>
              </div>
