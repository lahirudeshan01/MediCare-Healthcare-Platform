...
const services = {
  ...
  '/appointments':  process.env.DOCTOR_SERVICE_URL       || 'http://localhost:8082',
    ...
    createProxyMiddleware({
      ...
      on: {
        proxyReq: (proxyReq, req) => {
          console.log(`[GATEWAY] ${req.method} ${req.originalUrl} → ${target}${route}${req.url}`);
        },
        error: (err, req, res) => {
          console.error(`[GATEWAY] Proxy error for ${req.originalUrl}:`, err.message);
          res.status(502).json({ message: 'Service unavailable.', service: route });
        },
      },
    })
  );
